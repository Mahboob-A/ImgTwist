...
class ProductCreateUpdateDeleteAPIView(APIView):
    ...
    def patch(self, request, product_id):
        ...
        try:
            ...
            return Response(
                {
                    "status": "error",
                    "detail": f"Product not found with the ID {product_id}",
                },
    ...
    def delete(self, request, product_id=None):
        ...
        return Response(
            {"status": "success", "detail": "Product deleted successfully"},
    ...
    def delete(self, request, product_id=None):
        ...
        if not image_id or not image_ids:
            return Response(
                {
                    "status": "error",
                    "detail": "Image IDs for Bulk Delete or An Image ID is required",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if image_id:
            try:
                ...
                return Response(
                    {"status": "success", "detail": "Image deleted successfully"},
                    status=status.HTTP_204_NO_CONTENT,
                )
                ...
                return Response(
                    {
                        "status": "error",
                        "detail": f"Image not found with the ID {image_id}",
                    },
                    status=status.HTTP_404_NOT_FOUND,
                )
        ...
        if image_ids:
            ...
            existing_images_ids = set(product.images.values_list("id", flat=True))
            existing_images_ids = [str(id) for id in existing_images_ids]
            invalid_ids = [
                iamge_id
                for iamge_id in image_ids
                if iamge_id not in existing_images_ids
            ]
            ...
            if invalid_ids:
                return Response(
                    {
                        "status": "error",
                        "detail": f"Image not related to {product.get_product_name} can not be deleted. Invalid image IDs are {invalid_ids}",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if images_delate_count == 0:
                return Response(
                    {
                        "status": "error",
                        "detail": f"No images found with the IDs {image_ids} to Delete",
                    },
                    status=status.HTTP_404_NOT_FOUND,
                )
            ...
            return Response(
                {
                    "status": "success",
                    "detail": f"{images_delate_count} images deleted successfully",
                },
                status=status.HTTP_204_NO_CONTENT,
            )
...
class CategoryAPIView(APIView):
    ...
    def get(self, request, category_id=None):
        ...
        if category_id:
            try:
                ...
                return Response(
                    {"status": "error", "detail": "Category not found."},
                    status=status.HTTP_404_NOT_FOUND,
                )
        else:
    ...
    def delete(self, request, category_id=None):
        ...
        try:
            ...
            return Response(
                {"status": "success", "detail": "Category deleted."},
                status=status.HTTP_204_NO_CONTENT,
            )
            ...
            return Response(
                {
                    "status": "error",
                    "detail": "Something unexpected occured eleting category.",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
